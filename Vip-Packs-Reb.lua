...
VIP_Tab1:Button({
    Title = "Teleport To Best Farm",
    Desc ="Teleport To The Best Place For Farming",
    Callback = function()
        local o = function(p)
...
VIP_Tab1:Button({
    Title = "FPS Frames",
    Desc = "Without Any Lag And Super Boost",
    Callback = function()
...
VIP_Tab1:Input({
    Title = "Set Size",
    Desc = "Fill In The Size You Want",
...
VIP_Tab1:Toggle({
    Title = "Auto Size",
    Desc = "Size According To The Input You Entered",
...
local createdPlatforms = {} -- Tabel untuk menyimpan referensi lantai agar bisa dihapus

-- TOGGLE 1: Membuat Lantai/Platform
VIP_Tab1:Toggle({
    Title = "Fly Place",
    Desc = "Fly Mode Step 1",
    Callback = function(state)
        if state then
            -- Daftar posisi koordinat (X, Z) untuk setiap platform
            local posisiLantai = {
                Vector3.new(0, 150, 0),       -- Tengah
                Vector3.new(2000, 150, 0),    -- Kanan
                Vector3.new(-2000, 150, 0),   -- Kiri
                Vector3.new(0, 150, 2000),    -- Depan
                Vector3.new(0, 150, -2000),   -- Belakang
                Vector3.new(2000, 150, -2000),-- Kanan Belakang
                Vector3.new(2000, 150, 2000), -- Kanan Depan
                Vector3.new(-2000, 150, -2000),-- Kiri Belakang
                Vector3.new(-2000, 150, 2000)  -- Kiri Depan
            }

            for _, posisi in pairs(posisiLantai) do
                local lantai = Instance.new("Part")
                lantai.Name = "LantaiUtama_VIP"
                lantai.Shape = Enum.PartType.Block
                lantai.Anchored = true
                lantai.CanCollide = true
                lantai.Transparency = 1 -- Set ke 0 jika ingin terlihat
                lantai.Size = Vector3.new(2000, 5, 2000)
                lantai.Position = posisi
                lantai.Color = Color3.fromRGB(80, 80, 80)
                lantai.Material = Enum.Material.Concrete
                lantai.Parent = Workspace
                table.insert(createdPlatforms, lantai)
            end
        else
            -- Menghapus semua platform saat toggle OFF
...
VIP_Tab1:Toggle({
    Title = "Mode Fly OverPower",
    Desc = "Fly Mode Step 2 (You Must Activate Step 1 First)",
    Callback = function(state)
        ...
        if _G.AutoSizeEnabled then
            ...
            local function runLogic(character)
                ...
                -- 2. Auto Size 100 (Hanya Sekali)
                local remote = game:GetService("ReplicatedStorage"):FindFirstChild("rEvents") 
                               and game:GetService("ReplicatedStorage").rEvents:FindFirstChild("changeSpeedSizeRemote")
                if remote then
                    remote:InvokeServer("changeSize", 100)
                end
                task.wait(0.2)

                -- 3. Mulai Melingkar (Orbit)
                -- (Logika orbit diaktifkan via RunService di bawah)
                ...
                -- Pastikan priority tinggi agar tidak tertimpa animasi jalan
                task.defer(function()
                    task.wait(0.1)
                ...
                -- 5. Equip Weight (Tanpa batalkan emote)
                ...
                if weightItem then
                    ...
                    -- Menggunakan cara manual agar animasi tidak reset
                    weightItem.Parent = character 
                end
            end

            -- Jalankan urutan logika
            ...
            -- Koneksi Heartbeat untuk gerakan melingkar
...
VIP_Tab1:Toggle({
    Title = "Mode Save Zone",
    Desc = "Save Area (Mybe Hahahaha)",
